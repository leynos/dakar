...
repository-layout guidance; the [developer's guide](developers-guide.md) covers
build and contribution workflows instead.
...
- `typos.toml` is regenerated by `make spelling` from the live shared
  dictionary and the `typos.local.toml` overlay. Never edit it by hand.
